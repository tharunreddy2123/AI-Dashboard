...
try {
    ...
    $response = Invoke-RestMethod -Uri "http://localhost:8000/api/chat" -Method Post -Body $body -ContentType "application/json" -TimeoutSec 30
...
Write-Host ""
Write-Host "3. Testing cluster health endpoint..." -ForegroundColor Yellow
try {
    $stopwatch = [System.Diagnostics.Stopwatch]::StartNew()
    $response = Invoke-RestMethod -Uri "http://localhost:8000/api/openshift/cluster-health" -Method Get -TimeoutSec 30
    $stopwatch.Stop()
    ...
    Write-Host "  - GOOGLE_API_KEY not set in backend/.env" -ForegroundColor White
